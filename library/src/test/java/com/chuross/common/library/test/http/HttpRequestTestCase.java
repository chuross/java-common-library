...
public class HttpRequestTestCase {
    ...
    private void onHandle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        RequestPattern pattern =  method.equals("POST") || method.equals("PUT") ? new RequestPattern(target, IOUtils.toString(inputStream), getRequestHeader(request)) : new RequestPattern(target, getParameters(request), getRequestHeader(request));
